...
public:
    ...
    void Split()
    {
        ::QueryPerformanceCounter( &m_Split );
    }
    ...
    double Report() const
    {
        return ElapsedMicrosec( m_Start, m_End );
    }

    double ReportPreSplit() const
    {
        return ElapsedMicrosec( m_Start, m_Split );
    }

    double ReportPostSplit() const
    {
        return ElapsedMicrosec( m_Split, m_End );
    }
...
private:
    ...
    {
        return ( end.QuadPart - start.QuadPart ) * 1000000.0 / (double)m_Freq.QuadPart;
    }
};
...
public:
    ...
    void Start()
    {
        ...
        LONGLONG userTime;
        GetSystemTimes(
            reinterpret_cast<FILETIME*>(&m_StartIdle),
    ...
    void End()
    {
        ...
        LONGLONG userTime;
        GetSystemTimes(
            reinterpret_cast<FILETIME*>(&m_EndIdle),
            reinterpret_cast<FILETIME*>(&kernelTime),
            reinterpret_cast<FILETIME*>(&userTime)
            );
    ...
    void Split()
    {
        ...
        LONGLONG userTime;
        GetSystemTimes(
            reinterpret_cast<FILETIME*>(&m_SplitIdle),
            reinterpret_cast<FILETIME*>(&kernelTime),
            reinterpret_cast<FILETIME*>(&userTime)
            );
...
private:
    static double GetCpuTime(
        _In_ LONGLONG startIdle,
        _In_ LONGLONG endIdle,
        _In_ double startCpu,
        _In_ double endCpu
        )
    {
        return (100.0 - (((endIdle - startIdle) * 100) / (endCpu - startCpu))) * CpuCount();
    }
};
